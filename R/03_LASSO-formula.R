LASSO_formula_function <- function(Metabs_table, Traits_table, missing_threshold) {
  ...
  lasso_names_all <- setdiff(names(Penalized_na),
                             c("idno","TOM_ID","subject_id",
                               ...
                               "educ", "race", "gender", "exam", "site", "smoking", "DM", "race", "sidno", "redmeat"))
  


  f_all <- (paste(lasso_names_all, collapse = " + "))
  redmeat_formula_all <- as.formula(paste("redmeat ~", f_all))
